#!/usr/bin/env python3
"""NCP v4 seed 3 — OVH H100 training."""
import os
...
_name = _t.cuda.get_device_name(DEVICE_ID)
print(f"GPU {DEVICE_ID}: {_name}")
del _t
...
CKPT_TXT     = f"{WORK}/checkpoint_seed{SEED}.txt"
WEIGHTS_PATH = f"{WORK}/ncp_weights_seed{SEED}.pt"
os.makedirs(f"{WORK}/ckpts_seed{SEED}", exist_ok=True)
...
def _checkpoint(model, epoch):
    ...
    torch.save(model.state_dict(), f"{WORK}/ckpts_seed{SEED}/epoch_{epoch:03d}.pt")
    log.info("Checkpoint saved epoch %d seed %d", epoch, SEED)
...
if not all_feat_dfs: raise RuntimeError("No feature data")
log.info("Pass 1 done: %d tickers", len(all_feat_dfs))
...
log.info("Quartile panel: %d×%d", len(excess_df), len(excess_df.columns))
...
if not all_X: raise RuntimeError("No samples")
...
model=NCPTradingModel(
    ...
    sector_embedding_dim=config.SECTOR_EMBEDDING_DIM, dropout=config.DROPOUT,
).to(device)
...
log.info("Training epochs %d–%d", start_epoch+1, config.HISTORICAL_EPOCHS)
for epoch in range(start_epoch,config.HISTORICAL_EPOCHS):
    ...
    log.info("Epoch %d/%d | loss=%.4f | acc=%.4f | lr=%.2e",
        epoch+1,config.HISTORICAL_EPOCHS,total_loss/n,correct/n,scheduler.get_last_lr()[0])
    _checkpoint(model,epoch+1)

log.info("Done. Weights: %s", WEIGHTS_PATH)
print(f"\n=== SEED {SEED} COMPLETE — {WEIGHTS_PATH} ===")
